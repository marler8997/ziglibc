...
fn __main() callconv(.C) void {
    if (!sys.is_freestanding) {
        @This().stdin.fd = os.windows.peb().ProcessParameters.hStdInput;
        @This().stdout.fd = os.windows.peb().ProcessParameters.hStdOutput;
        @This().stderr.fd = os.windows.peb().ProcessParameters.hStdError;
    }
...
const ExitFunc = *const fn () callconv(.C) void;

// --------------------------------------------------------------------------------
// stdlib
// --------------------------------------------------------------------------------
usingnamespace if (sys.is_freestanding) struct {} else struct {
    comptime {
        if (@hasDecl(os, "exit")) @export(exit, .{ .name = "exit" });
    }
...
const global = struct {
    ...
    } ++ ([_]c.FILE{undefined} ** (max_file_count - 3))) else ([_]c.FILE{undefined} ** max_file_count);
...
usingnamespace if (sys.is_freestanding) struct {} else (struct {
    ...
    comptime {
        if (sys.supports_read) @export(getchar, .{ .name = "getchar" });
    }
    ...
    comptime {
        if (sys.supports_read) @export(getc, .{ .name = "getc" });
    }
    ...
    //}
    comptime {
        if (sys.supports_read) @export(fgetc, .{ .name = "fgetc" });
    }
    pub fn fgetc(stream: *c.FILE) callconv(.C) c_int {
        return getc(stream);
    }
    ...
    comptime {
        if (sys.supports_read) @export(_fread_buf, .{ .name = "_fread_buf" });
    }
    ...
    comptime {
        if (sys.supports_read) @export(fread, .{ .name = "fread" });
    }
    ...
    comptime {
        if (builtin.os.tag != .freestanding) @export(fopen, .{ .name = "fopen" });
    }
    ...
    comptime {
        if (builtin.os.tag != .freestanding) @export(fseek, .{ .name = "fseek" });
    }
    ...
    comptime {
        if (builtin.os.tag != .freestanding) @export(rewind, .{ .name = "rewind" });
    }
    ...
    comptime {
        if (sys.supports_write) @export(fputc, .{ .name = "fputc" });
    }
    ...
    comptime {
        if (sys.supports_write) @export(_fwrite_buf, .{ .name = "_fwrite_buf" });
    }
    ...
    comptime {
        if (sys.supports_write) @export(fwrite, .{ .name = "fwrite" });
    }
    ...
    comptime {
        if (sys.supports_write) @export(puts, .{ .name = "puts" });
    }
    ...
    comptime {
        if (sys.supports_write) @export(fputs, .{ .name = "fputs" });
    }
    ...
    comptime {
        if (sys.supports_read) @export(fgets, .{ .name = "fgets" });
    }
