...
fn relpath(comptime src_path: []const u8) []const u8 {
    ...
    const T = struct {
        ...
        const path = root ++ std.fs.path.sep_str ++ src_path;
    };
    return T.path;
}

/// Provides a _start symbol that will call C main
pub fn addZigStart(builder: *std.build.Builder, target: std.zig.CrossTarget, optimize: std.builtin.OptimizeMode) *std.build.LibExeObjStep {
...
pub fn addLibc(builder: *std.build.Builder, opt: ZigLibcOptions) *std.build.LibExeObjStep {
    const name = switch (opt.variant) {
        .freestanding => "cguana-freestanding",
        .only_std => "c-only-std",
    ...
    const index = std.build.FileSource{ .path = relpath("src" ++ std.fs.path.sep_str ++ "lib.zig") };
    ...
    const include_freestanding = switch (opt.variant) {
        .only_std, .freestanding, .full => true,
        else => false,
    };
    modules_options.addOption(bool, "freestanding", include_freestanding);
